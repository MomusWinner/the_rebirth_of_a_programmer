...
def spare_Atsuko():
    ...
    choosing_an_action({"Удачи": lambda: print_message("Атсуко", "Я пошла"),
                        "Можно с тобой?": lambda: awkward_conversation})
...
def seer():
    ...
    print_message("Мудрец", "Я знал что ты придёшь. В пророчестве говорилось,\n "
                            "что к мудрецу придёт человек в футболке с ящерицей, которая держит пивом.")
    ...
    print_message("Мудрец", "Вернёмся к пророчетву держи 3 летнее молоко.")
    ...
    main_character.addItems("испорченное молоко", 999)
...
def fight_with_boss_scene():
    print_message("","Безымянный долго шёл по лесу в поиске короля демонов. Прошло около 3 часов.\n"
                     "В мысля он уже называл стрика разными плоxими словами.\n"
